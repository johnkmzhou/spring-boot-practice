...
public class FileUploadTest {
	...
	// TODO test with user and csrf token
	@Test
	public void shouldListAllFiles() throws Exception {
		given(storageService.loadAll()).willReturn(Stream.of(Paths.get("first.txt"), Paths.get("second.txt")));

		mvc.perform(get("/upload")).andExpect(status().isOk()).andExpect(model().attribute("files",
	...
	public void shouldSaveUploadedFile() throws Exception {
		...
		mvc.perform(fileUpload("/upload").file(multipartFile)).andExpect(status().isFound())
